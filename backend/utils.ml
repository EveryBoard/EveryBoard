...
module JSON = struct
  ...
  let for_enum (values : ('a * t) list) : ('a -> t) * (t -> ('a, string) result) =
    let inversed_values = List.map (fun (x, y) -> (y, x)) values in
    let enum_to_yojson v = List.assoc v values in
    let enum_of_yojson json = match List.assoc_opt json inversed_values with
      | Some v -> Ok v
...
module CryptoUtils = struct
  ...
  (** [public_key_of_certificate_string pem] extracts an RSA public key from the
      ...
      of failure. *)
  let public_key_of_certificate_string (pem : string) : public_key =
...
module FirestoreUtils = struct
  ...
  let path_in_project (path : string) : string =
