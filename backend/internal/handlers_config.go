...
func (h *Handlers) handleSubscribeConfigRoom(gameId model.GameID) error {
	...
	if !h.subscriptions.Subscribe(h.connection, uid, gameId, SubscriptionToConfigRoom) {
	...
	err := h.store.Transaction(func(store model.Store) error {
		...
		case model.StatusCreated, model.StatusConfigProposed:
			...
			if uid != configRoom.Creator.ID {
				...
				h.bufferBroadcastToConfigRoom(&buf, gameId, model.CandidateJoinedMessage{Candidate: h.user, Elo: elo.CurrentElo})
		...
		return model.ErrorInternal // Unexpected game status
	})
	if err != nil {
...
func (h *Handlers) handleProposeConfig(config model.ConfigProposal) error {
	...
	err := h.store.Transaction(func(store model.Store) error {
		...
		h.bufferBroadcastToConfigRoom(&buf, configRoom.ID, model.ConfigRoomUpdateMessage{GameID: configRoom.ID, ConfigRoom: *configRoom})
		return nil
	})
	if err != nil {
		return err
