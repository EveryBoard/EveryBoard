...
func (h *Handler) handleSubscribeGame(gameId model.GameID) error {
	...
	if !h.subscriptions.Subscribe(h.connection, h.user.ID, gameId, session.SubscriptionToGame) {
	...
	err := h.store.Transaction(func(store store.Store) error {
		...
		buf.addSend(h.connection, protocol.GameUpdateMessage{Game: *game})
		...
		if err = store.ApplyToGameEvents(gameId, func(event *model.GameEvent) error {
			buf.addSend(h.connection, protocol.GameEventMessage{Event: *event, ServerTime: NowFloat()})
			return nil
		}); err != nil {
			return err
		}
		...
		buf.addSend(h.connection, protocol.GameEventMessage{Event: syncEvent, ServerTime: NowFloat()})
		return nil
	})
	if err != nil {
...
func (h *Handler) doEndGame(getResult func(*model.MinimalUser, *model.MinimalUser) model.Result, extraEvent *model.EventPayload) error {
	...
	err := h.store.Transaction(func(store store.Store) error {
		...
		if extraEvent != nil {
			...
			h.bufferBroadcastToGame(&buf, game.GameID, protocol.GameEventMessage{Event: *event, ServerTime: NowFloat()})
		}
		...
		if err = rating.ComputeAndUpdateElos(store, configRoom.GameName, winner, loser, draw); err != nil {
			return err
		}
		...
		eventMessage := protocol.GameEventMessage{Event: *event, ServerTime: NowFloat()}
		h.bufferBroadcastToGame(&buf, game.GameID, protocol.GameUpdateMessage{Game: *game})
...
func (h *Handler) handleResign() error {
...
func (h *Handler) handleNotifyTimeout(timeoutedPlayer model.Player) error {
	return h.doEndGame(func(playerZero *model.MinimalUser, playerOne *model.MinimalUser) model.Result {
...
func (h *Handler) handleGameEnd(winner model.PlayerOrNone) error {
	return h.doEndGame(func(playerZero *model.MinimalUser, playerOne *model.MinimalUser) model.Result {
		switch winner {
...
func (h *Handler) addEvent(eventData model.EventPayload) error {
	...
	err := h.store.Transaction(func(store store.Store) error {
		...
		h.bufferBroadcastToGame(&buf, gameId, protocol.GameEventMessage{
			ServerTime: NowFloat(),
			Event:      *event,
		})
		return nil
...
func (h *Handler) handlePropose(proposition model.Proposition) error {
	return h.addEvent(model.EventDataRequest(proposition))
}

func (h *Handler) handleReject(proposition model.Proposition) error {
	return h.addEvent(model.EventDataReplyReject(proposition))
}

func (h *Handler) handleAccept(proposition model.Proposition) error {
	switch proposition {
	...
	case model.PropositionRematch:
		...
		err := h.store.Transaction(func(store store.Store) error {
			...
			h.bufferBroadcastToLobby(&buf, protocol.ConfigRoomUpdateMessage{
...
func (h *Handler) handleAddTime(kind model.AddTimeKind) error {
	return h.addEvent(model.EventDataAddTime(kind))
}

func (h *Handler) handleMove(move json.RawMessage) error {
