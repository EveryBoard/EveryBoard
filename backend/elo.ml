...
module EloCalculationService = struct

    let opponent_of (player : Player.t) : Player.t = match player with
        | Zero -> One
        | One -> Zero

    let new_elos (elo_entry : EloEntry.t) : EloInfoPair.t =
        let normal_elo_differences : EloDifferences.t = normal_elo_differences elo_entry in
        {
            player_zero_info =
    ...
    let normal_elo_differences (elo_entry : EloEntry.t) : EloDifferences.t =
        {
            player_zero =
                let player : Player.t = Zero in
    ...
    let normal_elo_difference (k : float) (w : float) (p : float) : float =
        k *. (w -. p)

    let actual_new_elo (old_elo : float) (elo_difference : EloDifferences.t) =
        let minimum_final_elo : float = 1.0 in
        let elo_difference: EloDifferences.t =
            if elo_difference < 0.0 && old_elo < 100.0 then
                0.0 (* Not removing Elo from player below 100 *)
            else if elo_difference < 0.0 then
    ...
    let w_from (winner : Winner.t) (player : Player.t) : float =
        match winner with
        | Draw -> 0.5
        | Zero when player = Zero -> 1.0
        | One when player = One -> 1.0
        | _ -> 0.0


    let k_from (number_of_games_played : int) : float =
        if number_of_games_played < 20 then 60.0
        else if number_of_games_played < 40 then 40.0
        else 20.0

    let winning_probability (elo_winner : float) (elo_loser : float) : float =
