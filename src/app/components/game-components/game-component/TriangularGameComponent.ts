    ...
    extends GameComponent<R, M, S, L>
{
    ...
    public getDownwardCoordinate(x: number, y: number): Coord[] {
        const left: number = this.SQUARE_SIZE * 0.5 * x;
        const middle: number = this.SQUARE_SIZE * 0.5 * (x + 1);
        const right: number = this.SQUARE_SIZE * 0.5 * (x + 2);
        const top: number = this.SQUARE_SIZE * y;
        const bottom: number = this.SQUARE_SIZE * (y + 1);
    ...
    public getDownwardPyramidCoordinate(x: number, y: number): string {
        const zx: number = this.SQUARE_SIZE * x / 2;
        const zy: number = this.SQUARE_SIZE * y;
        const UP_LEFT: string = zx + ', ' + zy;
        const UP_RIGHT: string = (zx+this.SQUARE_SIZE) + ', ' + zy;
        const DOWN_CENTER: string = (zx+(this.SQUARE_SIZE/2)) + ', ' + (zy+this.SQUARE_SIZE);
        const CENTER: string = (zx+(this.SQUARE_SIZE / 2)) + ', ' + (zy+(this.SQUARE_SIZE / 2));
        return UP_LEFT + ',' +
               DOWN_CENTER + ',' +
               CENTER + ',' +
               UP_LEFT + ',' +
    ...
    public getUpwardPyramidCoordinate(x: number, y: number): string {
        const zx: number = this.SQUARE_SIZE * x / 2;
        const zy: number = (y + 1) * this.SQUARE_SIZE;
        const DOWN_LEFT: string = zx + ', ' + zy;
        const DOWN_RIGHT: string = (zx + this.SQUARE_SIZE) + ', ' + zy;
        const UP_CENTER: string = (zx + (this.SQUARE_SIZE / 2)) + ', ' + (zy - this.SQUARE_SIZE);
        const CENTER: string = (zx + (this.SQUARE_SIZE / 2)) + ', ' + (zy- (this.SQUARE_SIZE / 2));
        return DOWN_LEFT + ',' +
               UP_CENTER + ',' +
               CENTER + ',' +
