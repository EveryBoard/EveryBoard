...
describe('OnlineGameWrapperComponent of Quarto:', () => {
    ...
    it('Should be able to prepare a started game for creator', fakeAsync(async() => {
        ...
        tick(1);
        expect(wrapper.currentPart.copy().listMoves).toEqual([]);
        expect(wrapper.currentPart.copy().listMoves).toEqual([]);
    ...
    it('Should allow simple move', fakeAsync(async() => {
        ...
        expect(wrapper.currentPart.copy().listMoves).toEqual([FIRST_MOVE_ENCODED]);
        expect(wrapper.currentPart.copy().turn).toEqual(1);

        // Receive second move
        await receiveNewMoves([FIRST_MOVE_ENCODED, 166]);

        expect(wrapper.currentPart.copy().turn).toEqual(2);
        expect(wrapper.currentPart.copy().listMoves).toEqual([FIRST_MOVE_ENCODED, 166]);
        tick(wrapper.maximalMoveDuration);
    }));
    ...
    it('Prepared Game for joiner should allow simple move', fakeAsync(async() => {
        ...
        expect(wrapper.currentPart.copy().listMoves).toEqual([FIRST_MOVE_ENCODED, move.encode()]);
        expect(wrapper.currentPart.copy().turn).toEqual(2);
    ...
    it('Move should trigger db change', fakeAsync(async() => {
        ...
        spyOn(partDAO, 'update').and.callThrough();
        await doMove(FIRST_MOVE, true);
        expect(wrapper.currentPart.copy().listMoves).toEqual([FIRST_MOVE.encode()]);
    ...
    it('Victory move from player should notifyVictory', fakeAsync(async() => {
        ...
        expect(componentTestUtils.findElement('#youWonIndicator')).toBeTruthy('Component should show who is the winner.');
    }));
    describe('Take Back', () => {
