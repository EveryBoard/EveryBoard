...
describe('OnlineGameWrapperComponent of Quarto:', () => {
    ...
    const CREATOR: IJoueur & AuthUser= {
        email: 'cre@tor',
        username: 'creator',
        pseudo: 'creator', // TODO: remove all appeareances of 'pseudo'
        state: 'online',
        verified: true,
    };
    const OPPONENT: IJoueur & AuthUser = {
        pseudo: 'firstCandidate', // TODO: also remove
    ...
    async function prepareComponent(initialJoiner: IJoiner): Promise<void> {
        ...
        await joueurDAO.set(OBSERVER.username, OBSERVER);
    ...
    it('Prepared Game for joiner should allow simple move', fakeAsync(async() => {
        await prepareStartedGameFor(OPPONENT);
        tick(1);
    ...
    describe('Time Management', () => {
        ...
        it(`should stop player's local chrono when global chrono reach end`, fakeAsync(async() => {
            await prepareStartedGameFor(CREATOR, true);
            tick(1);
            spyOn(wrapper, 'reachedOutOfTime').and.callThrough();
    ...
    describe('Non Player Experience', () => {
        it('Should not be able to do anything', fakeAsync(async() => {
            await prepareStartedGameFor({ username: OBSERVER.username, verified: true, email: 'observer@home' });
