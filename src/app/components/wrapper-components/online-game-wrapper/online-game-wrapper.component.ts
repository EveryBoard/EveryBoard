...
export class OnlineGameWrapperComponent extends GameWrapper<MinimalUser> implements OnInit, OnDestroy {
    ...
    private async onCurrentPartUpdate(update: Part): Promise<void> {
        ...
        switch (updateType) {
            ...
            case UpdateType.MOVE:
                this.msToSubstract = this.getLastUpdateTime(Utils.getNonNullable(oldPart), part, updateType);
    ...
    public canAskTakeBack(): boolean {
        ...
        if (this.currentPart == null) {
            return false;
        } else if (this.currentPart.data.turn <= this.observerRole.value) {
            return false;
    ...
    public async acceptTakeBack(): Promise<void> {
        ...
        await this.gameService.acceptTakeBack(this.currentPartId,
                                              Utils.getNonNullable(this.currentPart), player, this.msToSubstract);
