...
implements AI<M, S, AIDepthLimitOptions, C>
{
    ...
    : GameNode<M, S>
    {
        ...
        const possibleMoves: ImmutableSet<M> = this.getPossibleMoves(node, config);
    ...
    private getPossibleMoves(node: GameNode<M, S>, config: MGPOptional<C>): ImmutableSet<M> {
        const currentMoves: MGPOptional<ImmutableSet<M>> = this.getMoves(node);
        if (currentMoves.isAbsent()) {
            const moves: M[] = this.moveGenerator.getListMoves(node, config);
            this.setMoves(node, new ImmutableSet(moves));
            return new ImmutableSet(moves);
        } else {
    ...
    private setMoves(node: GameNode<M, S>, moves: ImmutableSet<M>): void {
        node.setCache(this.name + '-moves', moves);
    }

    private getMoves(node: GameNode<M, S>): MGPOptional<ImmutableSet<M>> {
