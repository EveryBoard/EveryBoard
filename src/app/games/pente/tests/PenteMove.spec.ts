...
describe('PenteMove', () => {
    describe('of', () => {
        ...
        it('should create the move when it is on the board', () => {
            expect(() => PenteMove.from(new Coord(4, 2))).not.toThrow();
        });
    });
    describe('toString', () => {
        it('should be defined', () => {
            const move: PenteMove = PenteMove.from(new Coord(2, 3));
    ...
    describe('equals', () => {
        ...
        it('should return false for another move', () => {
            const move: PenteMove = PenteMove.from(new Coord(2, 3));
            const otherMove: PenteMove = PenteMove.from(new Coord(4, 2));
            expect(move.equals(otherMove)).toBeFalse();
        });
    });
    describe('encoder', () => {
        it('should be bijective', () => {
            EncoderTestUtils.expectToBeBijective(PenteMove.encoder, PenteMove.from(new Coord(2, 3)));
        });
    });
});
