    ...
    extends RectangularGameComponent<R, MancalaMove, MancalaState, number, MancalaConfig>
{
    ...
    public override async updateBoard(triggerAnimation: boolean): Promise<void> {
        ...
        if (triggerAnimation) {
            ...
            Utils.assert(this.nodeVanJaaj().parent.isPresent(), 'triggerAnimation in store should be false at first turn');
            this.changeVisibleState(this.nodeVanJaaj().parent.get().gameState);
            let indexDistribution: number = 0;
            const move: MancalaMove = this.nodeVanJaaj().previousMove.get();
