...
import { MGPOptional, ImmutableSet, MGPValidation, TimeUtils, Utils } from '@everyboard/lib';
    ...
    extends RectangularGameComponent<R, MancalaMove, MancalaState, number, MancalaConfig>
{
    ...
    private getStoreOwner(coord: Coord): MGPOptional<ImmutableSet<Player>> {
        return MancalaRules.FAKE_STORE_COORD.reverse().get(coord);
    }

    private getSpaceOwner(coord: Coord): Player {
        const owner: MGPOptional<ImmutableSet<Player>> = this.getStoreOwner(coord);
        if (owner.isPresent()) {
