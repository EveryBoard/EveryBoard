...
fdescribe('TriGoComponent', () => {
    ...
    describe('hoshi', () => {
        ...
        it('should be in (3, 3) and other centraly symmetrical coords for 13x13 board', fakeAsync(async() => {
            // Given a 13x13 board
            const customConfig: MGPOptional<TriGoConfig> = MGPOptional.of({ size: 13 });
            const state: GoState = TriGoRules.get().getInitialState(customConfig);
        ...
        it('should be in (2, 2) and other centraly symmetrical coords for 9x9 board', fakeAsync(async() => {
            // Given a 9x9 board
            const customConfig: MGPOptional<TriGoConfig> = MGPOptional.of({ size: 9 });
            const state: GoState = TriGoRules.get().getInitialState(customConfig);
        ...
        it('should not have a tengen when board has an even width and height', fakeAsync(async() => {
            ...
            const customConfig: MGPOptional<TriGoConfig> = MGPOptional.of({ size: 10 });
            const state: GoState = TriGoRules.get().getInitialState(customConfig);
