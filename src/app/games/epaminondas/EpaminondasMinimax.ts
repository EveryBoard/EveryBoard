...
export class EpaminondasMinimaxQ extends EpaminondasMinimax {
    public getBoardValue(move: EpaminondasMove, slice: EpaminondasPartSlice): NodeUnheritance {
        const zerosInFirstLine: number = slice.count(Player.ZERO, 0);
        const onesInLastLine: number = slice.count(Player.ONE, 11);
        if (slice.turn % 2 === 0) {
        ...
        let score1: number = 0;
        for (let y: number = 0; y < 12; y++) {
            score0 += slice.count(Player.ZERO, y) * (11-y)*(11-y);
            score1 += slice.count(Player.ONE, y) * y*y;
        }
        return new NodeUnheritance(score0 * Player.ZERO.getScoreModifier() +
            score1 * Player.ONE.getScoreModifier());

    }
}

export class EpaminondasMinimaxQ2 extends EpaminondasMinimax {
