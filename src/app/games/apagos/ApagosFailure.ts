export class ApagosFailure {
    ...
    public static readonly NO_PIECE_REMAINING_TO_DROP: () => string = () => $localize`There is no remaining piece of that color to drop!`;

    public static readonly MUST_END_MOVE_BY_DROP: () => string = () => $localize`Move must end by a drop, please select an arrow to drop or to finish a transfer!`;
}
