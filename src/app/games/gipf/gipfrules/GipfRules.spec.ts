...
describe('GipfRules:', () => {
    ...
    it('should start with an empty board position', () => {
        ...
        board.forEachCoord((_: Coord, content: GipfPiece) => {
            expect(content).toEqual(GipfPiece.EMPTY); // TODO: changed
        });
    });
    ...
    it('should force to capture consecutive pieces', () => {
        ...
        for (const [line, capturePositions] of linesAndCaptures) {
            ...
            const move: GipfMove = new GipfMove(new GipfPlacement(new Coord(0, -3), HexaDirection.DOWN_RIGHT, false),
                                                [capture],
                                                []);
            const legality: GipfLegalityStatus = rules.isLegal(move, slice);
            expect(legality.legal.isSuccess()).toBeFalse();
