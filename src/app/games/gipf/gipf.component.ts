                                                          ...
                                                          GipfLegalityInformation>
{
    ...
    private readonly movePhase: WritableSignal<number> = signal(GipfComponent.PHASE_PLACEMENT_COORD);
    ...
    protected override async showLastMove(move: GipfMove): Promise<void> {
        ...
        if (move.placement.direction.isPresent()) {
            ...
            this.inserted.set(
                MGPOptional.of(this.arrowToward(lastPlacement.coord, lastPlacement.direction.get())),
            );
        }
    }
    ...
    private async selectCapture(coord: Coord): Promise<MGPValidation> {
        ...
        this.constructedState.set(GipfRules.applyCapture(capture, this.constructedState()));
    ...
    private moveToPlacementPhase(): MGPValidation {
        this.movePhase.set(GipfComponent.PHASE_PLACEMENT_COORD);
        return MGPValidation.SUCCESS;
    }
    ...
    private moveToInitialCaptureOrPlacementPhase(): MGPValidation {
        ...
        if (this.possibleCaptures().length === 0) {
            ...
            this.movePhase.set(GipfComponent.PHASE_INITIAL_CAPTURE);
        }
        return MGPValidation.SUCCESS;
    }
    ...
    private computeArrows(placement: Coord): void {
        const arrows: Arrow<HexaDirection>[] = [];
    ...
    private async selectPlacementCoord(coord: Coord): Promise<MGPValidation> {
        ...
        this.placementEntrance.set(MGPOptional.of(coord));
    ...
    private async selectPlacementDirection(dir: MGPOptional<HexaDirection>): Promise<MGPValidation> {
        this.placement.set(
            MGPOptional.of(new GipfPlacement(this.placementEntrance().get(), dir)),
        );
