...
export class DvonnRules extends Rules<DvonnMove, DvonnState> {
    ...
    private static getFreePieces(state: DvonnState): Coord[] {
        ...
        return state.hexaBoard.getAllPieces()
            .filter((c: Coord): boolean =>
                state.hexaBoard.getAt(c).belongsTo(state.getCurrentPlayer()) &&
                state.hexaBoard.numberOfNeighbors(c) < 6);
    }
    ...
    public static getScores(state: DvonnState): number[] {
        ...
        state.hexaBoard.getAllPieces().map((c: Coord) => {
            const stack: DvonnPieceStack = state.hexaBoard.getAt(c);
    ...
    public isMovablePiece(state: DvonnState, coord: Coord): MGPValidation {
        ...
        if (state.hexaBoard.numberOfNeighbors(coord) >= 6) {
    ...
    private sourceCoords(state: DvonnState): Coord[] {
        return state.hexaBoard.getAllPieces()
            .filter((c: Coord): boolean =>
                state.hexaBoard.getAt(c).containsSource());
    }
    private markPiecesConnectedTo(state: DvonnState, coord: Coord, markBoard: boolean[][]) {
        ...
        HexaBoard.neighbors(coord, 1).forEach((c: Coord) => {
            if (state.hexaBoard.isOnBoard(c) && !markBoard[c.y][c.x] && !state.hexaBoard.getAt(c).isEmpty()) {
    ...
    private removeDisconnectedPieces(state: DvonnState): DvonnState {
        ...
        let newBoard: DvonnBoard = state.hexaBoard;
        state.hexaBoard.getAllPieces().forEach((c: Coord) => {
    ...
    : DvonnState
    {
        if (move === DvonnMove.PASS) {
            return new DvonnState(state.hexaBoard, state.turn + 1, true);
        } else {
            // To apply a legal move, the stack is added in the front of its end coordinate
            // (and removed from its start coordinate)
            const stack: DvonnPieceStack = state.hexaBoard.getAt(move.coord);
            const targetStack: DvonnPieceStack = state.hexaBoard.getAt(move.end);
            const newBoard: DvonnBoard = state.hexaBoard
