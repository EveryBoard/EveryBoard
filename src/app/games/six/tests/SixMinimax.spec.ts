...
describe('SixMinimax', () => {
    ...
    describe('pre-victories', () => {
        it('Should pass forcing move to children node to minimise calculations', () => {
            ...
            rules.node = new SixNode(MGPOptional.empty(), null, state);
        ...
        it('shound only count one preVictory when one coord is a forcing move for two lines', () => {
            ...
            const boardValue: { value: number, preVictory?: Coord } =
                minimax.getBoardValue(new MGPNode(MGPOptional.empty(), move, state));
    ...
    describe('Phase 2', () => {
        ...
        it('Score after 40th turn should be a substraction of the number of piece', () => {
            ...
            expect(minimax.getBoardNumericValue(new MGPNode(MGPOptional.empty(), SixMove.fromDrop(new Coord(1, 1)), state))).toBe(2);
        });
    });
});
