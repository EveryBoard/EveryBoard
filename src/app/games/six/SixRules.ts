...
export class SixRules extends AlignementMinimax<SixMove, SixGameState, SixLegalityStatus, SixVictorySource> {
    ...
    private getLineInfoOnHalfAnAxis(dir: HexaDirection,
                                    LAST_ENNEMY: Player,
                                    lastDrop: Coord,
                                    state: SixGameState)
                                    : BoardInfo
    {
        const testedCoords: number = 1;
        let subSum: number = 0;
        const contiguouslyConnected: Coord[] = [lastDrop];
        let testedCoord: Coord = lastDrop.getNext(dir, 1);
        while (testedCoords < 6) {
            const testedPiece: Player = state.getPieceAt(testedCoord);
            if (testedPiece === LAST_ENNEMY) {
                return {
                    status: null, // should not be used
                    preVictory: null, // either
                    sum: subSum,
                    victory: null,
                };
            }
